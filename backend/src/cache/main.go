...
func main() {
	...
	flag.StringVar(&params.dbUser, "db_user", "", "Database user name.")
	...
	// Set defaults or enforce required flags based on driver.
	if params.dbHost == "" {
		switch params.dbDriver {
	...
	if params.dbUser == "" {
		...
		case mysqlDBDriverDefault:
			params.dbUser = "root"
		case pgxDBDriverDefault:
			log.Fatal("--db_user is required when --db_driver=pgx")
		}
	}
