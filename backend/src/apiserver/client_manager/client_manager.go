...
func InitDBClient(initConnectionTimeout time.Duration) (*sql.DB, sqldrv.DBDialect, func() bool) {
	...
	if settings.Enabled {
		dialector = initDBDriverWithProvider(driverName, settings, initConnectionTimeout)
	} else {
...
func initDBDriver(driverName string, initConnectionTimeout time.Duration) string {
	...
	//
	// This is the one place where the pre-provider path does not behave exactly
	// as it did: a CREATE DATABASE refused for lack of privilege used to retry
	// until the timeout and then terminate, and now starts the server with a
	// warning when the database turns out to exist and be reachable. The
	// database user that cannot create databases is not specific to credential
	// providers -- a pre-provisioned schema and a least-privilege user are how
	// any managed database is usually run -- so the two paths share the
	// behavior rather than making it a reward for opting in. Every other
	// failure still terminates, as it did.
	drvDialect := sqldrv.NewDBDialect(driverName)
	warning, ensureErr := dbcreds.EnsureDatabase(dbName, initConnectionTimeout,
		func() error {
			_, execErr := db.Exec(fmt.Sprintf("CREATE DATABASE %s", drvDialect.QuoteIdentifier(dbName)))
			return execErr
		},
		func(err error) error { return ignoreAlreadyExistError(drvDialect, err) },
...
func initDBDriverWithProvider(driverName string, settings dbcreds.Settings, initConnectionTimeout time.Duration) gorm.Dialector {
	...
	case dbcreds.DriverMySQL:
		return initMySQLDriverWithProvider(settings, initConnectionTimeout)
	case dbcreds.DriverPostgreSQL:
		return initPostgreSQLDriverWithProvider(settings, initConnectionTimeout)
	default:
		glog.Fatalf("Driver %v is not supported, use \"mysql\" for MySQL, or \"pgx\" for PostgreSQL", driverName)
		return nil
	}
...
func initMySQLDriverWithProvider(settings dbcreds.Settings, initConnectionTimeout time.Duration) gorm.Dialector {
	dbName := common.GetStringConfig(mysqlDBName)
	provider, ctx := newDBCredentialProvider(settings, common.GetStringConfigWithDefault(mysqlPassword, ""))
	target := mysqlTargetFromConfig(settings)
	...
	prepareDatabaseWithProvider(ctx, bootstrap, provider, target, dbName, sqldrv.NewDBDialect(dbcreds.DriverMySQL), initConnectionTimeout)
...
func initPostgreSQLDriverWithProvider(settings dbcreds.Settings, initConnectionTimeout time.Duration) gorm.Dialector {
	dbName := common.GetStringConfig(postgresDBName)
	provider, ctx := newDBCredentialProvider(settings, common.GetStringConfigWithDefault(postgresPassword, ""))
	target := postgresTargetFromConfig(settings)
	...
	util.TerminateIfError(err)
	defer bootstrap.Close()
	prepareDatabaseWithProvider(ctx, bootstrap, provider, target, dbName, sqldrv.NewDBDialect(dbcreds.DriverPostgreSQL), initConnectionTimeout)
...
// credential.
func mysqlTargetFromConfig(settings dbcreds.Settings) dbcreds.Target {
	...
	return dbcreds.Target{
		...
		Params: params,
		TLS:    settings.TLSOptions(),
	}
}

// postgresTargetFromConfig describes the PostgreSQL endpoint to connect to.
func postgresTargetFromConfig(settings dbcreds.Settings) dbcreds.Target {
	params := map[string]string{}
	for key, value := range common.GetMapConfig(postgresExtraParams) {
		params[key] = value
	}
	...
	return dbcreds.Target{
		...
		User:   common.GetStringConfigWithDefault(postgresUser, "user"),
		Params: params,
		TLS:    settings.TLSOptions(),
	}
}
...
// that will not be used. The password is the caller's, because which one
// applies depends on the engine it has already resolved.
func newDBCredentialProvider(settings dbcreds.Settings, password string) (dbcreds.Provider, context.Context) {
	if warning := settings.IgnoredPasswordWarning(password); warning != "" {
		glog.Warning(warning)
	}

	provider, err := settings.NewProvider(password)
	util.TerminateIfError(err)
...
func prepareDatabaseWithProvider(ctx context.Context, bootstrap *sql.DB, provider dbcreds.Provider, target dbcreds.Target, dbName string, dialect sqldrv.DBDialect, initConnectionTimeout time.Duration) {
	warning, err := dbcreds.EnsureDatabase(dbName, initConnectionTimeout,
		func() error {
			_, execErr := bootstrap.Exec(fmt.Sprintf("CREATE DATABASE %s", dialect.QuoteIdentifier(dbName)))
			return execErr
		},
...
func postgresDataSourceName(dbName string) string {
	config, _, err := commonsql.CreatePostgreSQLConfig(
		common.GetStringConfigWithDefault(postgresUser, "user"),
		common.GetStringConfigWithDefault(postgresPassword, "password"),
		common.GetStringConfigWithDefault(postgresHost, "postgresql"),
		dbName,
