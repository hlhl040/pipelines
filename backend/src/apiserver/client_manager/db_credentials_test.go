...
func TestMySQLTargetMatchesLegacyConfig(t *testing.T) {
	...
	legacy := client.CreateMySQLConfig("root", "", "mysql", "3306", dbName, "1024", map[string]string{})
...
// PostgreSQL installation gets: unencrypted, which is what the legacy
// sslmode=disable connection string produces.
func TestPostgresTargetMatchesLegacyDefaults(t *testing.T) {
	config, err := dbcreds.PostgreSQLConfig(postgresTargetFromConfig())
	require.NoError(t, err)
...
// TestProbeSharesTheApplicationConnection pins that the probe authenticates as
// the same principal as the connection it vouches for. Probing as a different
// user would either mask a real failure, when that user happens to exist, or
// manufacture one when it does not.
func TestProbeSharesTheApplicationConnection(t *testing.T) {
	setConfig(t, map[string]interface{}{
		postgresHost: "aurora-pg.example.com",
		postgresPort: 5433,
	})
	...
	const dbName = "mlpipeline"
	assert.Equal(t, postgresDataSourceName(dbName), targetDataSourceName("pgx", nil, dbName),
		"the probe must use the application connection string")
}

// The MySQL probe names the target database on the connection the bootstrap
// step deliberately leaves unnamed.
func TestMySQLProbeNamesTheTargetDatabase(t *testing.T) {
	config, err := dbcreds.MySQLConfig(mysqlTargetFromConfig())
	require.NoError(t, err)
	require.Empty(t, config.DBName, "the bootstrap connection names no database")

	assert.Contains(t, targetDataSourceName("mysql", config, "mlpipeline"), "/mlpipeline")
}
